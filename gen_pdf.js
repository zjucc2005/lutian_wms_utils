...
const gen_pdf_material_label_batch = (options) => {
    ...
    for (let i = 0 ;i < options.length; i++) {
        ...
        while (opt.print_copies > 0) {
            ...
            f.line(97, 3, 97, 67)
            // 二维码
            if (opt._qr) f.addImage(opt._qr, 'png', 59.6, 4, 36.4, 36.4)
            // 文本
            f.setFontSize(12) // 设置固定字段名称
            f.text('供应商：', 4, 11)
            f.text('入库时间：', 4, 23.8)
            ...
            for (let k of Object.keys(meta)) {
                f.setFontSize(std_font_size)
                if (f.getTextWidth(opt[k]) > meta[k].l) {
                    let cur_font_size = std_font_size * meta[k].l / f.getTextWidth(opt[k])
                    f.setFontSize(cur_font_size)
                    f.text(opt[k], meta[k].x, meta[k].y - (std_font_size - cur_font_size) * 0.13)
                } else {
...
const gen_pdf_mo_picking = (options) => {
    ...
    // body
    let table_head = [['序号', '物料编码', '物料名称', '规格型号', '分子', '单位', '数量', '拣选数量', '拣选地址', '工序地址']]
    let table_body = []
    ...
    table_body.push([
        { content: '', styles: { lineWidth: 0, minCellHeight: 10 } },
        { content: '物流配送员：', styles: { lineWidth: 0, valign: 'bottom' } }, 
        { content: '', colSpan: 2, styles: { lineWidth: 0 } },
        { content: '产线对接人：', colSpan: 3, styles: { lineWidth: 0, valign: 'bottom'  } },
