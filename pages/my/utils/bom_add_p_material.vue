        ...
        methods: {
            ...
            handle_data() {
                ...
                for (let i=0;i < this.raw_data.length;i++) {
                    ...
                    if (i == 0) {
                        // 继承标题
                        this.done_data.push(['父项物料编码(脚本处理)', ...row])
                    } else {
                        if (row[0] === '0') {
                            h = { '1': row[1] }
                            // this.done_data.push(['', ...row])
                        } else {
                            h[row[0]] = row[1]
                            let arr = row[0].split('.')
                            let p_level = arr.slice(0, arr.length-1).join('.')
                            this.done_data.push([h[p_level], ...row])
            ...
            export_excel() {
                ...
                link.download = `BOM父级物料补充_${Date.now()}.xlsx`
                link.click()
