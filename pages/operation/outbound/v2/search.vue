            ...
            <uni-list-item
                ...
                <template v-slot:footer>
                    <view class="uni-list-item__foot">
                        <text>{{ _calc_complete_qty(obj) }} / {{ obj.base_unit_qty }} {{ obj.base_unit_name }}</text>
                        <progress 
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async handle_search(e) {
                ...
                if (this.search_form.bill_no) {
                    ...
                    if (this.search_form.bill_no.match(/^\d+$/)) {
                        this.search_form.bill_no = 'FHTZD' + this.search_form.bill_no // 自动补充前缀
                    }
            ...
            _calc_complete_qty(obj) {
                let complete_qty = 0
                this.inv_plans.forEach(inv_plan => {
                    if (inv_plan.FMaterialId == obj.material_id && inv_plan.FDocumentStatu == 'C') {
                        complete_qty += inv_plan.FOpQTY
                    }
                })
                return complete_qty
            },
            _calc_percentage(obj) {
                let complete_qty = this._calc_complete_qty(obj)
                return (complete_qty / obj.base_unit_qty) * 100
            },
            _calc_progress(inv_plans=[]) {
                ...
                let complete_percentage = Math.floor(complete_qty / outbound_qty * 100)
                this.goods_nav.options[0].info = `${complete_percentage}%`
